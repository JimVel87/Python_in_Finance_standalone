...
def Basel_2_min_cap_SA(asset_value, asset_type):
    ...
    """
    # Input control has been moved to calc_req_SA()
    CAR = 0.08;
...
def Basel_2_min_cap_IRB(PD, LGD, EAD):
    ...
    """
    # Input controls have been moved to calc_req_IRB()
    return PD * LGD * EAD;
...
# The calculation function (input control from Basel_2_min_cap_SA() has been moved here)
def calc_req_SA():
    if float(val_entry.get()) < 0:
        req_val_SA.config(text = 'Asset value cannot be negative!');
        return None;
...
# The calculation function (input controls from Basel_2_min_cap_IRB() have been moved here)
def calc_req_IRB():
    PD = float(PD_val.get());
    LGD = float(LGD_val.get());
    EAD = float(EAD_val.get());
    if PD > 1 or PD < 0:
        req_val_IRB.config(text = 'Probability of default must be between 0 and 1!');
        return None;
    if LGD > 1 or LGD < 0:
        req_val_IRB.config(text = 'Loss given default must be between 0 and 1!');
        return None;
    if EAD < 0:
        req_val_IRB.config(text = 'Exposure at default cannot be negative!');
        return None;
